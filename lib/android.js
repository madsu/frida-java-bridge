...
function _getApi () {
  const vmModules = Process.enumerateModules()
    .filter(m => /^lib(art|dvm)d.so$/.test(m.name))
...
function tryGetEnvJvmti (vm, runtime) {
  ...
  vm.perform(() => {
    const ensurePluginLoaded = new NativeFunction(
      Module.getExportByName('libartd.so', '_ZN3art7Runtime18EnsurePluginLoadedEPKcPNSt3__112basic_stringIcNS3_11char_traitsIcEENS3_9allocatorIcEEEE'),
      'bool',
...
function tryDetectJniIdsIndirectionOffset () {
  const impl = Module.findExportByName('libartd.so', '_ZN3art7Runtime12SetJniIdTypeENS_9JniIdTypeE');
  if (impl === null) {
    return null;
  }
...
`;
  ...
  const getOatQuickMethodHeaderImpl = Module.findExportByName('libartd.so',
    (pointerSize === 4)
...
function instrumentArtMethodInvocationFromInterpreter () {
  ...
  for (const exp of Module.enumerateExports('libartd.so').filter(exp => artInterpreterDoCallExportRegex.test(exp.name))) {
...
function ensureArtKnowsHowToHandleReplacementMethods (vm) {
  ...
  if (exportName !== null) {
    Interceptor.attach(Module.getExportByName('libartd.so', exportName), artController.hooks.Gc.copyingPhase);
  }
}
...
class JdwpSession {
  constructor () {
     ...
     */
    const acceptImpl = Module.getExportByName('libartd.so', '_ZN3art4JDWP12JdwpAdbState6AcceptEv');
    const receiveClientFdImpl = Module.getExportByName('libartd.so', '_ZN3art4JDWP12JdwpAdbState15ReceiveClientFdEv');
